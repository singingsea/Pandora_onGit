...
V4 update (2019-12-31)
    1. for auto read in header part, now we do not need worry about the column number.
    2. auto read in the OF file used by L1, to retrive filter info.

V3 update (2019-10-26):
    1. for OFFAXIS data, if we want use auto reference mode, the viewing elevation must be 90 degree. This version will try to fix the angles in OFFAXIS modes.The tollerance I set to 90 degree is <=3, and for other elevations <=0.5 degree.
    2. for U340 data, one must check the OF files to make sure the index for U340 and U340+DIFF are correct! I moved this inputs to user inputs part to avoid users overlook this issue. One can make the file to read in OF and auto determin this position, but I would like to leave it as this to make sure user is awared that the instrument filter wheel settings could be changed over time.
    3. include ZENITH data into OFFAXIS data (to provide more reference); include 90 degree elevation measurement in OFFAXIS data into ZENITH data (to increase the number of ZS measurements)
...
Warning: 
    ...
    3. note for V2 and V3 update, I did not do anything for old lev2 data! Pandonia is working on harmonization of historical data, i.e., reformat old lev1 to L0. Thus, I won't update PanPS part in future.
...
#instrument_no = 123 # the instrument serieal number
instrument_no = 103 # the instrument serieal number
...
start_date = '2018-12-04' # use 'yyyy-mm-dd' format, this only used if process_all_files = False  
end_date = '2019-10-24' # use 'yyyy-mm-dd' format, this only used if process_all_files = False
...
L1_file_path = '\\\\WONTLABJ105957.ncr.int.ec.gc.ca\\D\\Pandora\\'  + str(instrument_no) + '\\Blick\\L1\\'
#L1_file_path = '\\\\WONTLABJ105896\\G\\Pandora\\'  + str(instrument_no) + '\\Blick\\L1\\'
#L1_file_path = 'C:\\Projects\\Pandora\\data\\L1_test\\' + str(instrument_no) + '\\'
#lev2_file_path = '\\\\WONTLABJ105896\\G\\Pandora\\'  + str(instrument_no) + '\\L2\\FortMcKay\\'
#L1_file_path = 'C:\\Users\\ZhaoX\\Documents\\paper\\Mao\\L1\\'

# provide the OF file path
OF_file_path = '\\\\WONTLABJ105957.ncr.int.ec.gc.ca\\D\\Pandora\\' + str(instrument_no) + '\\Blick\\operationfiles\\'
#OF_file_path = '\\\\WONTLABJ105896.ncr.int.ec.gc.ca\\G\\Pandora\\' + str(instrument_no) + '\\Blick\\operationfiles\\'

# provide the output file path
spe_file_path = '\\\\WONTLABJ105957.ncr.int.ec.gc.ca\\D\\Pandora\\' + str(instrument_no) + '\\Blick\\spe_BlickP_L1_v4\\'
#spe_file_path = '\\\\WONTLABJ105896.ncr.int.ec.gc.ca\\G\\Pandora\\' + str(instrument_no) + '\\Blick\\spe_BlickP_L1_v3\\'
#spe_file_path = 'C:\\Projects\\Pandora\\data\\L2_test\\' + str(instrument_no) + '\\spe_BlickP_L1_v3\\'
...
sites_list_LSC = {'Downsview': 'EST', 'FortMcKay': 'MST', 'Fort McKay': 'MST', 'StGeorge':'EST', 'Toronto' : 'EST', 'Egbert':'EST','Fairbanks':'America/Anchorage','StonyPlain': 'MST','PEARL': 'EST',}
...
def read_BlickP_L1(file_nm):
    ...
    while header_part_ind != 2: # the data part start after read in '----' twice
        ...
        if header.find('Instrument operation file used: ') != -1: # find the OF file used    
            OF_file_name = str(header[len('Instrument operation file used: '):]).strip()
        ...
        if read_column_names == True:
            start_of_header_str = header.strip().find(':') +2 # find the column, and we only register column name after that! --> to aviod changing to Column numbers in BlickP output
            column_names.append(header.strip()[start_of_header_str:]) # save column names           
    ...
    return df, instrument_no, location, lat, lon, alt, wv, spec_start_column, spec_end_column, OF_file_name
#%% read PanPS lev2 data
...
def check_ZS_modes(full_file_name, df):
   df_sp = df[['Two letter code of measurement routine','UT date and time for beginning of measurement, yyyymmddThhmmssZ (ISO 8601)']]
   df_sp['datetime'] = pd.to_datetime(df_sp['UT date and time for beginning of measurement, yyyymmddThhmmssZ (ISO 8601)'])
   TF = (df_sp['Two letter code of measurement routine'] == 'ZO') | (df_sp['Two letter code of measurement routine'] == 'ZU')
...
def QDOAS_ASCII_formater_header(df,process_PanPS_lev2):
        if process_PanPS_lev2 == False:
            ...
            column_nm_dict = {                  
                    'Two letter code of measurement routine':'measurement_routine',
                    'UT date and time for beginning of measurement, yyyymmddThhmmssZ (ISO 8601)':'ISO_time',
                    'Total duration of measurement set in seconds':'measurement_time',
                    'Data processing type index':'Data_processing_type',# this is Pandora L1 data type, 2 = direct-sun, 3 = direct-moon, 4 = zenith-sky, 6 = profile, 7 = almucantar
                    'Integration time [ms]':'Int_time',
                    'Effective position of filterwheel #1, 0=filterwheel not used, 1-9 are valid positions':'FW_1',
                    'Effective position of filterwheel #2, 0=filterwheel not used, 1-9 are valid positions':'FW_2',
                    'Pointing zenith angle in degree, absolute or relative (see next column), 999=tracker not used':'PZA',
                    'Zenith pointing mode: zenith angle is... 0=absolute, 1=relative to sun, 2=relative to moon':'ZPM',
                    'Pointing azimuth in degree, increases clockwise, absolute (0=north) or relative (see next column), 999=tracker not used':'PAA',
                    'Azimuth pointing mode: like zenith angle mode but also fixed scattering angles relative to sun (3) or moon (4)':'APM',                    
                    'Scale factor for data, to obtain unscaled output divide data by this number':'scale_factor',# for Blick1.5
                    'Level 1 data type, data are... 1=corrected count rate [s-1], 2=radiance [uW/m2/nm/sr], 3=irradiance [uW/m2/nm]':'data_type'# for Blick1.5
            ...
            column_nm_dict = {
                    ...
                    'UT date and time for beginning of measurement, yyyymmddThhmmssZ (ISO 8601)':'ISO_time',
                    'Total duration of measurement set in seconds':'measurement_time',
                    'Total duration of measurement set in seconds (=#, if the line is a comment line)':'measurement_time', # this is for old version of PanPS lev2 data, such as 2013-2014
                    'Integration time [ms]':'Int_time',
                    'Position of filterwheel #1, 0=filterwheel not used, 1-9 are valid positions':'FW_1',
                    'Position of filterwheel #2, 0=filterwheel not used, 1-9 are valid positions':'FW_2',
                    'Pointing zenith angle in degree, absolute or relative (see next column), 999=tracker not used':'PZA',
                    'Zenith pointing mode: zenith angle is... 0=absolute, 1=relative to sun, 2=relative to moon':'ZPM',
                    'Pointing azimuth in degree, increases clockwise, absolute (0=north) or relative (see next column), 999=tracker not used':'PAA',
                    'Azimuth pointing mode: like zenith angle mode but also fixed scattering angles relative to sun (3) or moon (4)':'APM',
                    'Scale factor for output data, to obtain unscaled output devide data by this number':'scale_factor',
                    'Scale factor for output data (to obtain unscaled output devide data by this number)' : 'scale_factor',# this is for old version of PanPS lev2 data, such as 2013-2014
                    'Level 2 data type, data are... 1=corrected count rate [s-1], 2=irradiance [mW/m2/nm], 3=radiance [mW/m2/nm/sr]':'data_type'
                    }    
        df_sp = pd.DataFrame()
...
#                df_output.iat[i,col_nm_VEA] = 'NaN'
        ...
        # if VEA is close to 90 (less than 3 degree difference), fix it to 90   
        elev_tolerance = 0.5
        elev_tolerance_90 = 3 # Pandora has more issue with its 90 degree measurements
        for i in range(len(df_output)):           
            if abs(df_output.iat[i,col_nm_VEA] - 90) < elev_tolerance_90:
                df_output.iat[i,col_nm_VEA] = 90
            elif abs(df_output.iat[i,col_nm_VEA] - 50) < elev_tolerance:
                df_output.iat[i,col_nm_VEA] = 50
            elif abs(df_output.iat[i,col_nm_VEA] - 40) < elev_tolerance:
                df_output.iat[i,col_nm_VEA] = 40
            elif abs(df_output.iat[i,col_nm_VEA] - 30) < elev_tolerance:
                df_output.iat[i,col_nm_VEA] = 30
            elif abs(df_output.iat[i,col_nm_VEA] - 20) < elev_tolerance:
                df_output.iat[i,col_nm_VEA] = 20
            elif abs(df_output.iat[i,col_nm_VEA] - 15) < elev_tolerance:
                df_output.iat[i,col_nm_VEA] = 15
            elif abs(df_output.iat[i,col_nm_VEA] - 10) < elev_tolerance:
                df_output.iat[i,col_nm_VEA] = 10
            elif abs(df_output.iat[i,col_nm_VEA] - 8) < elev_tolerance:
                df_output.iat[i,col_nm_VEA] = 8
            elif abs(df_output.iat[i,col_nm_VEA] - 5) < elev_tolerance:
                df_output.iat[i,col_nm_VEA] = 5
            elif abs(df_output.iat[i,col_nm_VEA] - 3) < elev_tolerance:
                df_output.iat[i,col_nm_VEA] = 3
            elif abs(df_output.iat[i,col_nm_VEA] - 2) < elev_tolerance:
                df_output.iat[i,col_nm_VEA] = 2
            elif abs(df_output.iat[i,col_nm_VEA] - 1) < elev_tolerance:
                df_output.iat[i,col_nm_VEA] = 1
...
#%% read OF file
def read_OF_file(OF_file_path, OF_file_name):
    
    file_nm = OF_file_path + OF_file_name
    print('     Read OF file ... ' + OF_file_name) 
    f = open(file_nm,'r')
    idx_U340 = np.NaN
    dx_U340_DIFF = np.NaN
    
    while True: # the data part start after read in '----' twice
        line = f.readline() # read in a line of the file
        if not line:
            break

        if line.find(' -> U340\n') != -1: # find U340 filter position line
            print('       -> Found : ' + line.strip())     
            idx_U340 = int(line[line.find(' -> U340\n')-1:line.find(' -> U340\n')])
        if line.find(' -> U340+DIFF\n') != -1: # find U340 filter position line
            print('       -> Found : ' + line.strip())
            idx_U340_DIFF = int(line[line.find(' -> U340+DIFF\n')-1:line.find(' -> U340+DIFF\n')])

    f.close()           
    
    if np.isnan(idx_U340):   
        print('Warnning: U340 filter position is not found in ' + OF_file_path + OF_file_name) 

    if np.isnan(idx_U340_DIFF):   
        print('Warnning: U340 + DIFF filter position is not found in ' + OF_file_path + OF_file_name)         
              
    return idx_U340, idx_U340_DIFF
        ...
        if (full_file_name.find('Pandora' + str(instrument_no)) != -1): # only get files that have name "PandoraXXX" and "L1"
            if not os.path.getsize(full_file_name) == 0: # only copy non-zero size L0 file
                if process_this_L1_file == True:
                    ...
                    if process_PanPS_lev2 == False:
                        df, instrument_no_infile, location, lat, lon, alt , wv, spec_start_column, spec_end_column, OF_file_name = read_BlickP_L1(full_file_name) # read L1 file
                        idx_U340, idx_U340_DIFF = read_OF_file(OF_file_path, OF_file_name) # read OF file
                            ...
                            for Measurement_Type in Measurement_Types:   
                                ...
                                for FW2_Type in FW2_Types:
                                    ...
                                    if sum(TF) !=0: # if a combination of measurement type and FW2 position is not empty, then we will write it to an spe file
                                        ...
                                        if (FW2_Type == idx_U340) | (FW2_Type == idx_U340_DIFF): # note, depends on the OF files for earch instrument, filter wheel #2 may used different U340 filters, so one must check this and provide correct info. 
                                        ...
                                        if Measurement_Type == 'OFFAXIS': # if we write OFFAXIS data, we also want include ZENITH data into it. This will give more reference
                                            # get header part of one type of measurement
                                            df_header_subset = df_header[(df_header.Measurement_Type == Measurement_Type) | (df_header.Measurement_Type == 'ZENITH')][df_header.FW2 == FW2_Type]
                                            # get spectrum part of one type of measurement
                                            df_spec_subset = df_spec[(df_header.Measurement_Type == Measurement_Type) | (df_header.Measurement_Type == 'ZENITH')][df_header.FW2 == FW2_Type]
                                            df_header_subset.Measurement_Type = 'OFFAXIS' # for those zenith-sky measurements, we want include them to OFFAXIS data!
                                        elif Measurement_Type == 'ZENITH': # if we write ZENITH data, we also want include OFFAXIS data that made with 90 degree measurements into it. This will give more measurements
                                            # get header part of one type of measurement
                                            df_header_subset = df_header[(df_header.Measurement_Type == Measurement_Type) | (df_header.Measurement_Type == 'OFFAXIS')][df_header.FW2 == FW2_Type][df_header.VEA == 90]
                                            # get spectrum part of one type of measurement
                                            df_spec_subset = df_spec[(df_header.Measurement_Type == Measurement_Type) | (df_header.Measurement_Type == 'OFFAXIS')][df_header.FW2 == FW2_Type][df_header.VEA == 90]
                                            df_header_subset.Measurement_Type = 'ZENITH' # for those OFFAXIS measurements made with elev = 90, we want include them to ZENITH data!                                            
                                        else:
                                            # get header part of one type of measurement
